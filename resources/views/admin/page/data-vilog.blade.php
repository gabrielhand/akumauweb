...
@section('content')
        ...
        <div class="flex justify-end text-sm breadcrumbs mb-4 mt-2">
            <ul>
                ...
                <li onclick="window.location.href='{{ route('pesanan') }}'"
                    class="{{ request()->is('data/dmvilog') ? 'text-purple-400 cursor-pointer' : '' }}">Pesanan</li>
                <li>Data DM Vilog</li>
            </ul>
        </div>
        <div>
            <div id="tabel-semuavilog" class="border light-dark-text shadow-sm p-2 rounded-xl border-none bg-invoice-card">
                <div>
                    <p class="lg:text-2xl text-lg light-dark-text font-semibold m-4">Daftar Semua Pesanan DM Vilog</p>
                    <div class="px-4 my-2">
                        <input type="text" name="" id="searchInput" placeholder="Cari Data DM Vilog..."
                            class="light-dark-border w-full input-tbl-sp rounded-lg lg:text-base text-sm px-3 py-2">
                    </div>
                </div>
                <div class="relative overflow-x-auto overflow-y-hidden tbl-semuavilog">
                            ...
                            @else
                                @foreach ($data as $datas)
                                    <tr id="tr-tbl-sp" data-status="{{ $datas->status }}" class="border-tbl-riwayat">
                                        ...
                                        <td class="p-4 align-middle">
                                            {{ $datas->password_vilog }}
                                        </td>
                                        <td class="p-4 align-middle">
                                            {{ $datas->loginvia_vilog }}
                                        </td>
                                        <td class="p-4 align-middle">
                                            <div class="dropdown">
                                                ...
                                                <ul tabindex="0"
                                                    ...
                                                    <li>
                                                        <div tabindex="0" role="button"
                                                            onclick="window.location.href='/dmvilog-status/{{ $datas->order_id }}/Success'"
                                                    ...
                                                    <li>
                                                        <div tabindex="0" role="button"
                                                            onclick="window.location.href='/dmvilog-status/{{ $datas->order_id }}/Proses'"
                                                            class="rounded-lg px-3 py-2 hover:bg-slate-200/60 duration-300 cursor-pointer focus:bg-purple-500 focus:text-white">
                                                            Proses
                                                        </div>
                                                    </li>
                                        ...
                                        <td class="p-4 align-middle">
                                            <div class="btn btn-error"
                                                onclick="window.location.href='/dmvilog/hapus/{{ $datas->id }}'">Hapus
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
    ...
    <script>
        document.getElementById('searchInput').addEventListener('input', function() {
            ...
            let rows = document.querySelectorAll('#tabel-semuavilog tbody tr');
