<section id="best-seller" class="md:px-6 px-4">
    <h1 class="lg:text-xl md:text-lg sm:text-sm font-semibold mb-5">⭐ Paling Banyak Dicari</h1>
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        @php $count = 0 @endphp
        @foreach ($kategori as $jsgori)
            @if ($jsgori->populer == '1' && $count < 8)
                <div
                    class="border shadow-sm relative w-full h-16 md:h-24 border-none rounded-xl cursor-pointer flex flex-col justify-center p-0 text-white bg-[#A600FF]">
